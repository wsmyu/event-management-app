...
const GuestManagementPage = () => {
  ...
  const handleSendInvitation = (userId) => {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error('Failed to manage guest invitation');
      }
      return response.json();
    })
    .then(() => fetchGuestList()) // Refresh the guest list
    .catch(error => console.error('Error managing guest invitation:', error));
};
